...
#define kalloc_size(addr)                       0
#define kfree_addr(addr)                      ::free(addr)
...
//		os_panic("OSRuntime: C++ runtime failed to initialize.");
//	}
	...
	for (section = firstsect(segment);
		...
		if (textStart) {
			...
			for (lim = num_structors, baseIdx = 0; lim; lim >>= 1) {
				...
				if (!value) {
					os_panic("%s: null structor", kmodInfo->name);
				}
...
{
	os_panic("Size of array created by new[] has overflowed");
}
};
