...
public:
	...
	closeReadGate()
	{
		pthread_rwlock_rdlock(poolGate);
	}

	inline void
	openReadGate()
	{
		pthread_rwlock_unlock(poolGate);
	}
	...
	closeWriteGate()
	{
		pthread_rwlock_wrlock(poolGate);
	}
...
OSSymbolPool::init()
{
    ...
    poolGate = new pthread_rwlock_t;
    pthread_rwlock_init(poolGate, nullptr);
...
OSSymbolPool::OSSymbolPool(const OSSymbolPool *old)
{
	...
	buckets = old->buckets;
    poolGate = (pthread_rwlock_t*) malloc(sizeof(pthread_rwlock_t));
}
...
OSSymbolPool::~OSSymbolPool()
{
	...
	if (poolGate) {
		pthread_rwlock_destroy(poolGate);
        free(poolGate);
	}
}
...
OSSymbolPool::removeSymbol(OSSymbol *sym)
{
	...
	if (!j) {
		...
		os_panic("removeSymbol %s count %d ", sym->string ? sym->string : "no string", count);
		return;
	}
 ...
 *********************************************************************
 */
OSDefineMetaClassAndStructorsWithInit(OSSymbol, OSString, OSSymbol::initialize())
