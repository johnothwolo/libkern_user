...
enum {
    kOSRegistryModsMode = 1
};

int gIOKitDebug = kOSRegistryModsMode;
...
//			os_panic((char*)"Trying to change a collection in the registry");
            printf("panic!!!!!!!\n");
            printf("Trying to change a collection in the registry");
            abort();
		} else {
			printf("Trying to change a collection in the registry");
		}
