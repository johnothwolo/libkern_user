...
#define kext_free(a,b)     ::free((void*)(a))

typedef struct thread_call *thread_call_t;
typedef struct pmap             *pmap_t;

int kext_alloc(vm_address_t *result, unsigned size, bool fixed __unused)
{
    *result = (vm_address_t) malloc(size);
    return 0;
}
...
extern "C" pthread_spinlock_t           vm_allocation_sites_lock;
static IOSimpleLock           * sKextAccountsLock = (IOSimpleLock*)&vm_allocation_sites_lock;
...
OSKext::initialize(void)
{
	...
	setResult = sKernelKext->infoDict->setObject("CFBundleIdentifier", // kCFBundleIdentifier
	...
	setResult = sKernelKext->infoDict->setObject("CFBundleVersionKey" , //kCFBundleVersionKey,
	    scratchString);
	assert(setResult);
	...
	assert(scratchString);
	setResult = sKernelKext->infoDict->setObject("CFBundleNameKey", // kCFBundleNameKey
	    scratchString);
	assert(setResult);
	OSSafeReleaseNULL(scratchString);
...
#error arch
	...
	sect = (kernel_section_t *)getsectbyname("__PRELINK", "__symtab");
	...
	OSString     * aPath)
{
	...
	if (infoDict) {
		os_panic("Attempt to set info dictionary on a kext "
	...
	bool     externalDataIsMkext)
{
	...
	if (infoDict->getObject(_kOSKextExecutableKey) ||
		...
		os_panic("Attempt to set an executable on a kext "
		    "that already has one (%s).",
		    getIdentifierCString());
		goto finish;
	}
...
{
	if (isLoaded()) {
		os_panic("Attempt to free loaded kext %s.", getIdentifierCString());
	}
...
z_alloc(void * notused __unused, u_int num_items, u_int size)
{
	...
	if (total > UINT32_MAX) {
		os_panic("z_alloc(%p, %x, %x): overflow caused by %x * %x\n",
	...
	if (allocSize64 > UINT32_MAX) {
		os_panic("z_alloc(%p, %x, %x): overflow caused by %x + %lx\n",
	...
	OSCollectionIterator *  theIterator )
{
	...
	while ((deviceTreeName =
		...
		if (myBundleID &&
			...
			if (!updated) {
				/* 25322874 */
				os_panic("Missing OSKextExcludeList dictionary\n");
			}
			break;
		}
	} // while ( (deviceTreeName = ...) )
...
{
	if (linkedExecutable) {
		os_panic("Attempt to set linked executable on kext "
		    "that already has one (%s).\n",
		    getIdentifierCString());
	}
...
OSKext::updateLoadedKextSummaries(void)
{
	...
	if (IORecursiveLockHaveLock(sKextLock) == false) {
		os_panic("sKextLock must be held");
	}
#endif
